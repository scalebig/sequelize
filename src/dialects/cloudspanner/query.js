...
const debug = logger.debugContext('sql:cloud-spanner');
  ...
  async run(sql, parameters) {
    ...
    try {
      if (this.options.type === QueryTypes.RAW) {
        const multiResultSet = await connection.updateSchema([sql]);
        results = multiResultSet[0];
      }
      
      if ([QueryTypes.SELECT, QueryTypes.VERSION, QueryTypes.SHOWTABLES].includes(this.options.type)) {
        const multiResultSet = await connection.run({ sql });
        results = multiResultSet[0];
      }
      console.log(results);
  ...
  formatResults(data) {
    ...
    if (this.isSelectQuery()) {
      ...
      if (this.options.raw === false && this.sequelize.options.quoteIdentifiers === false) {
        ...
        // data = data.map(dataItem => _.reduce(dataItem, (prev, value, key) => {
      ...
      const mappedData = data?.map(dataItem => dataItem.toJSON()) || [];
      return this.handleSelectQuery(mappedData);
    }
